...
const RuleSchema = z.object({
  ...
  metric:              z.enum(['daily_spend', 'weekly_spend', 'monthly_spend']).default('daily_spend'),
...
router.post('/rules', requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO alert_rules (org_id, provider, metric, threshold_usd, discord_webhook_url, debounce_hours)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [orgId, data.provider, data.metric, data.threshold_usd, data.discord_webhook_url, data.debounce_hours]
    );
