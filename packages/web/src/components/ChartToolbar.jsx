...
function HelpPopover({ chartView }) {
  ...
  return (
    <div className="dash-chart-rail-hint" ref={ref}>
      <button
        type="button"
        className="dash-chart-rail-hint-btn"
      ...
      {open && (
        <div className="dash-chart-rail-hint-panel">
          <div className="dash-chart-rail-hint-row">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
// 2D/3D toggle. Laid out as a vertical rail down the chart card's left edge so
// the chart body keeps the card's full height — a horizontal bar on top stole
// ~40px of chart height on every viewport. Collapsing it (Dashboard.jsx)
// removes the rail from the flex row entirely so the chart spans the full card.
  ...
  return (
    <div className="dash-chart-rail">
      ...
      <div className="dash-chart-rail-foot">
        ...
        <div className="obs-range-picker dash-chart-rail-views">
          <button
        ...
        <div className="dash-chart-rail-actions">
          <button className="obs-btn dash-chart-rail-refresh" onClick={onRefresh} disabled={syncing}>
