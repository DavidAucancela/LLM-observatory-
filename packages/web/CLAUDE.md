...
- `ChartToolbar.jsx` — controles del chart del Dashboard (título, leyenda de modelos, comparar vs. periodo anterior, toggle 3D/2D, refrescar, hint de uso). Es un **riel vertical** (`.dash-chart-rail`, 178px) pegado al borde **derecho** de `.dash-chart-card` (vía `flex-direction: row-reverse` en `.dash-chart-card` — el riel sigue siendo el primer hijo en el JSX, así que en móvil, que vuelve a `flex-direction: column`, aparece arriba en orden natural del DOM). En móvil (`max-width: 767px`) el riel vuelve a ser barra horizontal arriba, con la leyenda scrolleando en X.
