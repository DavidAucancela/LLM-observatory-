...
- **Server-side cost math goes through `packages/api/src/services/pricingBridge.js`** — the single adapter over `@llm-observatory/sdk`'s pricing tables + `calculate*` fns. Never re-add a local `PRICING` table in `packages/api` (`services/providerUsage.js` used to have one and it drifted). The bridge adds a provider→fn dispatch map, the Anthropic cache-write **1.25× surcharge** (`CACHE_CREATION_INPUT_MULTIPLIER`), and its own snapshot-suffix `normalizeModelId` (inlined, not imported from the SDK — see the npm-lag gotcha below). `services/llmJudge.js` keeps its own 5-line `JUDGE_MODEL` table on purpose (single model per provider, not a full surface).
- **`@llm-observatory/sdk` is an npm dependency of `packages/api`** (`"*"`), resolved by `npm install --production` in `packages/api/Dockerfile`. The **published npm version can lag the monorepo source** — the repo was left at `1.1.0` after adding `normalizeModelId`/Grok+Kimi tables, so prod ran an SDK without them until pricingBridge stopped depending on `sdk.normalizeModelId`. **Pending:** `npm publish` from `packages/sdk` (now `1.2.0`); after that the inlined `normalizeModelId` mirror in pricingBridge can be dropped and Grok/Kimi tables/`calculate*` fns will resolve in prod. The Dockerfile also `COPY scripts/ ./scripts/` so `packages/api/scripts/*` run in the container.
...
- [ ] **`npm publish` `@llm-observatory/sdk` 1.2.0** — deployed API runs the stale npm `1.1.0`; publish so Grok/Kimi tables + `normalizeModelId` resolve in prod, then drop the inlined mirror in `pricingBridge.js`
